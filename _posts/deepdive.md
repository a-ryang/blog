...
- 화살표함수는 프로토타입이 없고 생성자 함수로서 사용할 수 없다.
- 또 this 바인딩이 없어 자신을 둘러싼 lexical scope의 this가 사용되어 정적으로 결정된다.
- 초과된 인수는 arguments 내부 프로퍼티에 들어간다.
- arguments는 유사 배열 객체이지만 배열이 아니므로 배열 메서드를 사용할 수 없다.
